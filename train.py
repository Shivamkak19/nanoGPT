...
while True:
    ...
    # CHANGE: converted loss to base2, reporting perplexity in bpc
    if iter_num % eval_interval == 0 and master_process:
        losses = estimate_loss()
        # Convert loss from nats to bits
        train_bpc = losses["train"] / math.log(2)
        val_bpc = losses["val"] / math.log(2)
        # Calculate perplexity - can use torch.exp since exp(loss) = 2^(loss/log(2))
        ...
        if wandb_log:
            wandb.log(
                {
                    "iter": iter_num,
                    "train/loss-bpc": train_bpc,
                    "val/loss-bpc": val_bpc,
                    "train/perplexity-bpc": train_ppl,
                    "val/perplexity-bpc": val_ppl,
                    "lr": lr,
                    "mfu": running_mfu * 100,
                }
            )
